...
class PointControllerTest {
     ...
     * 포인트 내역이 없는경우 {@link NotFoundException} 이 발생합니다.
     * 리턴 값으론 ErrorResponse 를 반환합니다.
     * 검증
    ...
    /**
     * 사용자 포인트 충전 - 성공
     * 사용자가 유효한 값으로 포인트 충전시 충전된 포인트를 반환 합니다.
     *
     * 검증
     * - UserPoint 는 JSON 으로 반환되어야합니다.
     * - 올바른 요청일 경우 응답 값은 성공(200) 이어야합니다.
     * - 반환되는 포인트의 사용자 ID가 요청자의 ID와 동일해야합니다.
     */
    @DisplayName("유효한 사용자 ID로 포인트 충전 요청시, 충전된 사용자 포인트를 반환한다.")
    @Test
    void shouldReturnChargedPoint_whenCharge_withValidUserId() throws Exception {
        //given
        long userId = 1L, pointAmount = 10L;
        UserPoint userPoint = new UserPoint(userId, pointAmount, System.currentTimeMillis());
        given(chargePointUseCase.handle(new UserId(userId), pointAmount)).willReturn(userPoint);

        String body = new ObjectMapper().writeValueAsString(new PointChargeRequest(pointAmount));
    ...
    /**
     * 사용자 포인트 충전 - 실패
     * 사용자가 잘못된 포인트 수치로 충전을 요청할 경우 실패 응답 및 에러 코드를 반환합니다.
     *
     * 검증
     * - 포인트는 항상 양수여야합니다.
     * - 응답값은 JSON 이어야합니다.
     * - 예외 응답의 상태코드는 처리불가 (422) 여야합니다.
     * - 예외 응답 코드가 존재해야합니다.
     */
    @DisplayName("유효하지 않은 포인트(음수)로 포인트 충전 요청시, 충전 실패 예외와 예외 응답을 반환한다.")
    @Test
    void shouldReturnChargedPoint_whenCharge_withInvalidUserId() throws Exception {
        //given
        long userId = 1L , pointAmount = -1L;
        String body = new ObjectMapper().writeValueAsString(new PointChargeRequest(pointAmount));
